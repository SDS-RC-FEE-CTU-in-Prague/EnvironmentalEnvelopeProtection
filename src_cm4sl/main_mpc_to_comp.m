function [u_mpc] = main_mpc_to_comp(t, input, params, controller_on, points_x_r, points_x_l, points_y_r, points_y_l, obstacle_detected, ds_x, ds_y)

%           1   2  3  4    5       6       7      8
% input = beta, r, v, yaw, df_ref, u_prev, x_cor, y_cor
% params = [lf, lr, m, I, ha]
persistent opti r x0 u_prev v x u as k0L k1L k2L k3L sfr sfl radf1 radf2 dsx_1 dsx_2 dsy_1 dsy_2 radius_1 radius_2 sD obs_on_1 obs_on_2 caf car
...
wheel_dist_right = -ha;
dist_to_left = 3.5/2; % distance between the centerline to each borderline
dist_to_right = 3.5/2;
if (t >= 20 && t <= 23) % narrow part with unparallel road lines for LK test
    dist_to_left = 1.7/2;
    dist_to_right = 1.7/2;
end
rad_ius = 2;
...
if t == 0
    ...
    radf1 = opti.variable(N+1, 1);
    radf2 = opti.variable(N+1, 1);
    sD = opti.variable(N, 1);

    % weights
    steering_wheel_track = 1000;
...
%         end
        ...
        opti.subject_to(((yf-dsy_1)^2+(xf-dsx_1)^2) >= obs_on_1*(radius_1 + wheel_dist_left - radf1(k))^2);
        opti.subject_to(((yf-dsy_2)^2+(xf-dsx_2)^2) >= obs_on_2*(radius_2 + wheel_dist_left - radf2(k))^2);
        opti.subject_to(radf1(k) >= 0);
        opti.subject_to(radf2(k) >= 0);
        obj = obj + obs_on_1*Qo*radf1(k)^2 + obs_on_2*Qo*radf2(k)^2;

        % lane keeping
        yl = yf - (dist_to_left - wheel_dist_left);
        yr = yf + (dist_to_right + wheel_dist_right);
        opti.subject_to(k0L - roadPadding >= yl - k3L*xf^3 - k2L*xf^2 - k1L*xf - sfl(k))
        opti.subject_to(k0L + roadPadding <= yr - k3L*xf^3 - k2L*xf^2 - k1L*xf + sfr(k));
        opti.subject_to(sfl(k) >= 0);
        opti.subject_to(sfr(k) >= 0);
        % slack objective
        obj = obj + Qenv_ar*sfl(k)^2 + Qenv_ar*sfr(k)^2;
        obj = obj + 100*u(k)^2;
        obj = obj + 50*x(1,k)^2;
    end
...
%     %obstacle avoidance and lane keeping k+1
        ...
        yl = yf - (dist_to_left - wheel_dist_left);
        yr = yf + (dist_to_right + wheel_dist_right);
        opti.subject_to(k0L - roadPadding >= yl - k3L*xf^3 - k2L*xf^2 - k1L*xf - sfl(N+1))
        opti.subject_to(k0L + roadPadding <= yr - k3L*xf^3 - k2L*xf^2 - k1L*xf + sfr(N+1));
        opti.subject_to(((yf-dsy_1)^2+(xf-dsx_1)^2) >= obs_on_1*(radius_1 + wheel_dist_left - radf1(N+1))^2);
        opti.subject_to(((yf-dsy_2)^2+(xf-dsx_2)^2) >= obs_on_2*(radius_2 + wheel_dist_left - radf2(N+1))^2);
        opti.subject_to(radf1(N+1) >= 0);
        opti.subject_to(radf2(N+1) >= 0);
        obj = obj + obs_on_1*Qo*radf1(N+1)^2 + obs_on_2*Qo*radf2(N+1)^2;
    obj = obj + Qenv_ar*sfl(N+1)^2 + Qenv_ar*sfr(N+1)^2;
...
if(controller_on ~= 1 || 0 == input(3)) % check v_min and start_time boundary
    ...
    fl = polyfit((Xl+Xr)/2,(Yl+Yr)/2,3);
    % input parsing
    opti.set_value(v, input(3));
    opti.set_value(u_prev, input(6));
    opti.set_value(r, input(5));
    
    opti.set_value(k0L, fl(4));
    opti.set_value(k1L, fl(3));
    opti.set_value(k2L, fl(2));
    opti.set_value(k3L, fl(1));
