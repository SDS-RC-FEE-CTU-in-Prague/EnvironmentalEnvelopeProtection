...
persistent opti r x0 u_prev v x u as k0LL k1LL k2LL k0LR k1LR k2LR k0RL k1RL k2RL k0RR k1RR k2RR sfrl sfll srrl srll sfrr sflr srrr srlr sol sD caf car
...
if t == 0
    ...
    for k = 1:N
        ...
        % lane keeping
            ...
            opti.subject_to( k0LL - roadPadding >= yfl - k2LL*xfl^2 - k1LL*xfl - sfll(k));
            opti.subject_to( k0LR + roadPadding <= yfl - k2LR*xfl^2 - k1LR*xfl + sflr(k));
            opti.subject_to(k0RL - roadPadding >= yfr - k2RL*xfr^2 - k1RL*xfr - sfrl(k));
            opti.subject_to(k0RR + roadPadding <= yfr - k2RR*xfr^2 - k1RR*xfr + sfrr(k));
            % slack stuff
...
%   x(k+1)
    ...
    opti.subject_to( k0LL - roadPadding >= yfl - k2LL*xfl^2 - k1LL*xfl - sfll(N+1));
    opti.subject_to( k0LR + roadPadding <= yfl - k2LR*xfl^2 - k1LR*xfl + sflr(N+1));
    opti.subject_to(k0RL - roadPadding >= yfr - k2RL*xfr^2 - k1RL*xfr - sfrl(N+1));
    opti.subject_to(k0RR + roadPadding <= yfr - k2RR*xfr^2 - k1RR*xfr + sfrr(N+1));
...
if(controller_on ~= 1 || 0 == input(3)) % check v_min and start_time boundary
    ...
    %line fitting
%     main_left = polyfit(line_detection(:,1),line_detection(:,2),2);
    wheel_left_left = polyfit(line_detection(:,3)-3,line_detection(:,4),2);
    wheel_left_right = polyfit(line_detection(:,5)-3,line_detection(:,6),2);
    wheel_right_left = polyfit(line_detection(:,7)-3,line_detection(:,8),2);
    wheel_right_right = polyfit(line_detection(:,9)-3,line_detection(:,10),2);
    main_right = polyfit(line_detection(:,11),line_detection(:,12),2);
    % input parsing
    ...
    % In ruts
    opti.set_value(k0LL, wheel_left_left(3));
    opti.set_value(k1LL, wheel_left_left(2));
    opti.set_value(k2LL, wheel_left_left(1));
    opti.set_value(k0LR, wheel_left_right(3));
    opti.set_value(k1LR, wheel_left_right(2));
    opti.set_value(k2LR, wheel_left_right(1));
    
    opti.set_value(k0RL, wheel_right_left(3));
    opti.set_value(k1RL, wheel_right_left(2));
    opti.set_value(k2RL, wheel_right_left(1));
    opti.set_value(k0RR, wheel_right_right(3));
    opti.set_value(k1RR, wheel_right_right(2));
    opti.set_value(k2RR, wheel_right_right(1));
    
    % out ruts
%     opti.set_value(k0LL, wheel_left_right(3));
%     opti.set_value(k1LL, wheel_left_right(2));
%     opti.set_value(k2LL, wheel_left_right(1));
%     opti.set_value(k0LR, wheel_right_left(3));
%     opti.set_value(k1LR, wheel_right_left(2));
%     opti.set_value(k2LR, wheel_right_left(1));
%     
%     opti.set_value(k0RL, wheel_right_right(3));
%     opti.set_value(k1RL, wheel_right_right(2));
%     opti.set_value(k2RL, wheel_right_right(1));
%     opti.set_value(k0RR, main_right(3));
%     opti.set_value(k1RR, main_right(2));
%     opti.set_value(k2RR, main_right(1));
%     
