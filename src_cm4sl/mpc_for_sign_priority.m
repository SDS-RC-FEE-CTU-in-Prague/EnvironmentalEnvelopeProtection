function [u_mpc] = mpc_for_sign_priority(t, input, params, controller_on, points_x_r, points_x_l, points_y_r, points_y_l, obstacle_detected, ds_x, ds_y, SignDetections)

%           1   2  3  4    5       6       7      8
% input = beta, r, v, yaw, df_ref, u_prev, x_cor, y_cor
% params = [lf, lr, m, I, ha]
persistent opti r x0 u_prev v x u as k0L k1L k2L k3L k0R k1R k2R k3R sfr sfl srr srl radfl1 radfr1 radfl2 radfr2 dsx_1 dsx_2 dsy_1 dsy_2 radius_1 radius_2 sD obs_on_1 obs_on_2 caf car
...
if t == 0
    ...
    for k = 1:N
        ...
        obj = obj + obs_on_1*Qo*radfl1(k)^2 + obs_on_1*Qo*radfr1(k)^2 + obs_on_2*10^3*Qo*radfl2(k)^2 + obs_on_2*10^3*Qo*radfr2(k)^2;
...
%   x(k+1)
    ...
    obj = obj + obs_on_1*Qo*radfl1(N+1)^2 + obs_on_1*Qo*radfr1(N+1)^2 + obs_on_2*10^3*Qo*radfl2(N+1)^2 + obs_on_2*10^3*Qo*radfr2(N+1)^2;
...
if(controller_on ~= 1 || 0 == input(3)) % check v_min and start_time boundary
    ...
    if SignDetections(1) > 0 % small caution sign
        opti.set_value(radius_1,1+0.5);
        opti.set_value(dsx_1,SignDetections(1));
        opti.set_value(dsy_1,SignDetections(2));
    ...
    if SignDetections(3) > 0 % big caution sign
        opti.set_value(radius_2,1.5);
        opti.set_value(dsx_2,SignDetections(3));
        opti.set_value(dsy_2,SignDetections(4));
