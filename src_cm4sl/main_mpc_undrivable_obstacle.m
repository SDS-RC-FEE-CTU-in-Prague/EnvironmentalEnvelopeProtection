...
% params = [lf, lr, m, I, ha]
persistent opti r x0 u_prev v x u as k0L k1L k2L k3L k0R k1R k2R k3R sfr sfl srr srl radfl1 radfr1 radfl2 radfr2 dsx_1 dsx_2 dsy_1 dsy_2 radius_1 radius_2 sD obs_on_1 obs_on_2 caf car bigslack
...
if t == 0
    ...
    points_count = 1;
    bigslack = opti.variable(N+1, points_count);
    ...
    for k = 1:N
        ...
        obj = obj + obs_on_1*Qo*radfl1(k)^2 + obs_on_2*Qo*radfl2(k)^2 + obs_on_1*Qo*radfr1(k)^2 + obs_on_2*Qo*radfr2(k)^2;

        opti.subject_to((((yfl+yfr)/2-dsy_1)^2+((xfl+xfr)/2-dsx_1)^2) >= obs_on_1*(radius_1 - bigslack(k))^2);
        opti.subject_to((((yfl+yfr)/2-dsy_2)^2+((xfl+xfr)/2-dsx_2)^2) >= obs_on_2*(radius_2 - bigslack(k))^2);
        obj = obj + obs_on_1*Qo*bigslack(k)^2 + obs_on_2*Qo*bigslack(k)^2;
        
        % lane keeping
        opti.subject_to( k0L - roadPadding >= yfl - k3L*xfl^3 - k2L*xfl^2 - k1L*xfl - sfl(k));
        opti.subject_to(k0R + roadPadding <= yfr - k3R*xfr^3 - k2R*xfr^2 - k1R*xfr + sfr(k));
...
%   x(k+1)
    ...
    opti.subject_to( k0L - roadPadding >= yfl - k3L*xfl^3 - k2L*xfl^2 - k1L*xfl - sfl(N+1));
    opti.subject_to(k0R + roadPadding <= yfr - k3R*xfr^3 - k2R*xfr^2 - k1R*xfr + sfr(N+1));
    obj = obj + obs_on_1*Qo*radfl1(N+1)^2 + obs_on_2*Qo*radfl2(N+1)^2 + obs_on_1*Qo*radfr1(N+1)^2 + obs_on_2*Qo*radfr2(N+1)^2;
    obj = obj + Qenv_ar*sfl(N+1)^2 + Qenv_ar*sfr(N+1)^2+ Qenv_ar*srl(N+1)^2 + Qenv_ar*srr(N+1)^2;
    obj = obj + 50*x(1,N+1)^2;
    
    opti.subject_to((((yfl+yfr)/2-dsy_1)^2+((xfl+xfr)/2-dsx_1)^2) >= obs_on_1*(radius_1 - bigslack(N+1))^2);
    opti.subject_to((((yfl+yfr)/2-dsy_2)^2+((xfl+xfr)/2-dsx_2)^2) >= obs_on_2*(radius_2 - bigslack(N+1))^2);
    obj = obj + obs_on_1*Qo*bigslack(N+1)^2 + obs_on_2*Qo*bigslack(N+1)^2;
...
if(controller_on ~= 1 || 0 == input(3)) % check v_min and start_time boundary
    ...
    opti.set_value(k0R, fr(4));
    opti.set_value(k1R, fr(3));
    opti.set_value(k2R, fr(2));
    opti.set_value(k3R, fr(1));
